...
1. **Data Ingestion** - Move data from source systems into the data warehouse.

2. **Data Storage** - Store data in a format optimized for analytics and reporting.

3. **Data Processing** - Transform, clean, and prepare data into a format ready for analysis.

4. **Data Analysis & Delivery** - Analyze the processed data and deliver insights through reports, dashboards, and other business intelligence tools.

### Dimensional Modeling: Tables in a Data Warehouse
...
1. **Surrogate Key**
   - A unique integer generated automatically by the database when a row is inserted
   - Maintains consistency and accuracy within the warehouse

2. **Alternate Key**
   - A natural or business key from the source system (e.g., product code, customer ID)
   - Maintains traceability between the warehouse and source systems
...
1. **Full T-SQL support**
   - Write DDL and DML including MERGE for upsert scenarios

2. **Fully managed**
   - No infrastructure to configure; compute scales automatically and independently from storage

3. **OneLake integration**
   - Data stored in Delta format, accessible by other Fabric workloads without copying

4. **Cross-database querying**
   - Query across warehouses and lakehouses using three-part naming: `database.schema.table`

5. **Familiar tooling**
   - Connect via SSMS, Azure Data Studio, or any SQL client through standard TDS connections

6. **Copilot assistance**
   - Generates SQL from natural language, provides code completion, explains/fixes queries
...
### Ingesting Data into a Fabric Warehouse
...
- Good for one-off explorations (Best for doing a quick, one-time investigation or experiment.)
...
1. **Hide internal objects**
   - Remove staging tables, surrogate key columns, ETL artifacts from the field list

2. **Rename columns**
   - Use business-friendly names (e.g., rename `CustRgn` to `Customer Region`)

3. **Add descriptions**
   - Explain what tables and columns represent
...
1. **Cardinality**
   - How rows correspond between tables. In a star schema, fact-to-dimension relationships are typically **many-to-one** (many fact rows map to one dimension row)

2. **Cross-filter direction**
   - Which way filters propagate. **Single direction** (dimension filters the fact table) is the standard for star schemas — keeps behavior predictable and performant
...
1. **Views**
   - Standardize how T-SQL consumers access and query data — encapsulates JOINs, filters, column selections
   - **Consumer:** T-SQL analysts, report tools

2. **Measures**
   - Reusable DAX expressions defining calculations (totals, averages, ratios, counts) — create in the warehouse model view
   - **Consumer:** Power BI reports, dashboards
